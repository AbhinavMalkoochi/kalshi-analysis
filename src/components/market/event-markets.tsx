      ...
      <CardContent className="space-y-1.5 pt-0">
        {sortedMarkets.map((market) => {
          ...
          return (
            <div
              key={market.ticker}
              className={`flex items-center justify-between gap-2 rounded-lg border px-3 py-2 ${isCurrentMarket
        ...
        {markets.length > 10 && (
          <div className="w-full rounded bg-black/30 py-1.5 text-center text-xs text-muted-foreground">
            Showing all {markets.length} options
          </div>
        )}
      </CardContent>
    </Card>
